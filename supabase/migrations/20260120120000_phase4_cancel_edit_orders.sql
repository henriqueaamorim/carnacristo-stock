-- Fase 4: cancelar e editar pedido (admin) com estoque consistente
...
REVOKE ALL ON FUNCTION public.cancel_order_with_inventory_refund (uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.cancel_order_with_inventory_refund (uuid) TO authenticated;

-- ---------------------------------------------------------------------------
-- edit_order_with_inventory_adjustment
-- Reverte itens atuais ao estoque e aplica novo carrinho (pedido permanece completed).
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Nova composição do pedido (mesma lógica de baixa que create_order)
...
REVOKE ALL ON FUNCTION public.edit_order_with_inventory_adjustment (uuid, text, public.payment_method, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.edit_order_with_inventory_adjustment (uuid, text, public.payment_method, jsonb) TO authenticated;
